...
def _do_step(active, agents, by_family, by_work, by_school, by_age,
             ...
             beta_work=BETA_WORK, beta_school=BETA_SCHOOL,
             beta_random=BETA_RANDOM, beta_same_age=BETA_SAME_AGE):
    """Process one day. Returns (new_active, new_exposed_count).

    All beta_* parameters default to the module-level constants but can be
    overridden per call — sweep scripts should pass explicit values here
    rather than mutating the module constants (which silently fails for
    beta_family, since default args are bound once at definition time)."""
    for a in active:
