...
unchanged). The per-status-AR / peak-shape / SAR->beta helpers (sar_to_beta,
activity_category, per_status_ar, run_once_tracked, count_major_peaks, wpval)
are shared with full_sweep_50rep.py, beta_family_sweep.py, and
age_betawork_sweep.py via abm_sweep_common.py, rather than duplicated.
...
from abm_sweep_common import (sar_to_beta, per_status_ar, count_major_peaks,
                               wpval, run_once_tracked as _run_once_tracked)
...
def main():
    beta = sar_to_beta(SAR_TARGET, run_exp.GAMMA)  # = 0.0221 for SAR=20%
