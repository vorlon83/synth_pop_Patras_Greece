...
```bash
cd Thesis_Synthpop-main
python python_ipf/main_ipf_pipeline.py
cd ..
```

> **Run this from `Thesis_Synthpop-main/`, not from `python_ipf/`.** The script's marginal-input paths are relative to `Thesis_Synthpop-main/`; running it from inside `python_ipf/` fails with `FileNotFoundError` on the `marginal_distributions/*.csv` files.

**Runtime:** ~5 minutes.

**Output:** `Thesis_Synthpop-main/python_ipf/ipf_results/households.json`. Step 2 (`filter_patras.py`) reads this path directly, so no manual copy is required. Optionally keep a copy in the data folder:

```bash
cp Thesis_Synthpop-main/python_ipf/ipf_results/households.json data/synthpop/households.json
```

**Expected:** ~119,070 households / ~306,021 individuals (full Achaia). See the reproducibility note under Step 2 — the synthesis is seeded but not bit-identical across environments, so counts vary by up to ~0.2% at this stage.
...
> **Reproducibility note.** The household synthesis is stochastic. Although it is seeded (`seed=42`), it is **not bit-identical across Python/NumPy versions**: a fresh run reproduces a *demographically equivalent* population to within ~0.02% of the canonical Patras counts, with all `verify_population.py` structural gates passing (Step 3) — not an exact byte-for-byte copy. The SHA-256 below is therefore an **integrity checksum of the canonical `patras_households.json` shipped in this repository**, for confirming that file downloaded intact; it is *not* a value a fresh pipeline run is expected to match. To reproduce every downstream number exactly (GIS, ABM), use the shipped canonical file rather than a freshly synthesized one.

SHA-256 of the canonical file shipped in this repo (post-education re-weighting):
```
...
**Note:** this repo ships only the compiled `main.pdf`, not the LaTeX source, so the
`Synthetic_Population_manuscript/img/` output path below does not exist here. The scripts create
it automatically relative to the current working directory; if you want the figures written
somewhere specific, pass or edit the output path in the script, or just collect the PNGs from
wherever they land.
...
## Step 9 — Manuscript PDF

This repo does not include the LaTeX source, only the compiled result: `main.pdf` at the repo
root. There is nothing to compile here; regenerating figures per Step 8 changes the inputs a
LaTeX build would use, but recompiling the PDF itself requires the manuscript source, which
lives elsewhere.
...
| 9 — PDF | n/a — compiled `main.pdf` is included, no LaTeX source in this repo | — | — |
