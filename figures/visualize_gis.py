...
Both gis/assignment.py and generate_initial_state_geojson.py now shuffle the
household order with the same PIPELINE_SEED (default 42), so the two scripts
draw from an identical seeded household order and the raw (undeduplicated)
building x district assignment is bit-identical between them.

A small number of building footprints (~2-3%) intersect more than one
district polygon and appear as more than one row in the geojson, each with
its own independently-drawn household subset; gis/assignment.py's own
per-district CSV counts every such row, so every NUMERIC aggregate here
(district summary, household-size distribution, age pyramid, employment
shares) is computed from `df_raw` (all rows, undeduplicated) to match
gis_district_statistics.csv exactly. A deduplicated view (`bld`, one row per
osm_id) is used ONLY for the two spatial scatter maps (fig1, fig5 panels A/C)
so a boundary building is not plotted twice at (nearly) the same coordinates
-- a rendering-only choice with no effect on any reported count.
"""

import json, ast, warnings, os, sys

# Console output uses box-drawing / ellipsis characters; on a non-UTF-8 system
# codepage (e.g. Greek cp1253) print() raises UnicodeEncodeError partway
# through the run. Reconfigure stdout/stderr to UTF-8 (falls back silently on
# older Python without reconfigure) so this script always completes and saves
# all figures regardless of the console's default encoding.
...
# ── parse rows ───────────────────────────────────────────────────────────────────
# A small number of building footprints (~2-3%) intersect more than one district
# polygon (boundary/overlap cases) and therefore appear as more than one row in
# the geojson, each row carrying its OWN independently-assigned household subset
# -- gis/assignment.py (which owns the canonical data/gis_district_statistics.csv)
# counts every such row independently when summing district totals, so this
# script must do the same for any NUMERIC aggregate (district summary, household
# sizes, age pyramid, employment shares) or those aggregates will silently drop
# ~2-3% of the canonical households/people and no longer match
# gis_district_statistics.csv. `df_raw` (all rows, undeduplicated) is therefore
# the source of truth for every count/statistic below.
#
# For the two SPATIAL SCATTER maps only (fig1, fig5 panels A/C) a duplicated row
# would plot the same physical footprint twice at (nearly) the same coordinates
# -- purely a rendering artifact, not a data issue -- so `bld` (one representative
# row per osm_id, the one with the most households) is used for those maps only.
records = []
...
print(f"  {len(df_raw)} building x district rows (canonical-matching, undeduplicated)")
print(f"  {df_raw['n_hh'].sum()} total household assignments")
print(f"  {df_raw['n_people'].sum()} total people assigned")

# map-only deduplicated view: one row per physical building (osm_id), highest-
# occupancy duplicate kept, so a boundary building isn't drawn twice on a map.
...
print(f"  {len(bld)} unique building footprints (map rendering only)")

# ── person-level data ───────────────────────────────────────────────────────────
# Uses df_raw (all rows) so person counts / age pyramid / employment shares match
# gis_district_statistics.csv exactly -- see note above.
...
# Uses df_raw (undeduplicated) so n_buildings/n_hh/n_people match
# gis_district_statistics.csv's own per-district sums exactly (see note above).
dist = (df_raw[df_raw["DISTRICT_ID"].notna()]
        .groupby("DISTRICT_ID")
...
# Title counts use the canonical undeduplicated df_raw counts (matching
# gis_district_statistics.csv and every other reported figure/table in the
# manuscript), not len(occ)/len(unassigned): those are built from the
# deduplicated `bld` view used for scatter-plot rendering only, and differ
# from the canonical total by the ~2-3% of footprints that straddle a
# district boundary and are collapsed to one row for plotting (see module
# docstring). Using the dedup counts here previously produced a title that
# silently disagreed with the rest of the paper (31,780 vs. the canonical
# 32,734 occupied buildings).
n_occupied_canonical = int((df_raw["n_hh"] > 0).sum())
n_unoccupied_canonical = int((df_raw["n_hh"] == 0).sum())
ax.set_title("Patras GIS Assignment: Building footprints & school locations\n"
             f"({n_occupied_canonical:,} occupied | {n_unoccupied_canonical:,} unoccupied | {len(sch)} schools)",
             fontsize=11, pad=10)
...
ax_c.set_title(f"School locations by level\n({len(sch)} active schools)", fontsize=9, fontweight="bold")
...
# ══════════════════════════════════════════════════════════════════════════════
print("\n── Summary ──────────────────────────────────────────────")
print(f"Unique building footprints:      {len(bld):,}  (map-rendering view, one row per osm_id)")
print(f"Occupied building footprints:    {(bld['n_hh']>0).sum():,}")
print(f"Unoccupied building footprints:  {(bld['n_hh']==0).sum():,}")
print(f"Mean HH/building (occupied):     {bld[bld['n_hh']>0]['n_hh'].mean():.2f}")
print(f"Max HH/building:                 {bld['n_hh'].max()}")
print()
print(f"Total households assigned:       {df_raw['n_hh'].sum():,}  (canonical-matching, df_raw)")
print(f"Total people assigned:           {df_raw['n_people'].sum():,}  (canonical-matching, df_raw)")
print(f"Districts covered (DISTRICT_ID): {df_raw['DISTRICT_ID'].nunique()}")
