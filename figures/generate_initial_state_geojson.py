...
A fixed random seed is used for reproducibility of this specific regeneration
run (gis/assignment.py itself sets no seed, so exact per-district totals can
vary marginally run to run within the same overall assignment method; totals
land close to the canonical CSV because building capacity -- not household
supply -- is the binding constraint for most districts).
