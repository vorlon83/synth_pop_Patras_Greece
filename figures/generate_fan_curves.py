...
# Set household transmission rate for SAR=20%
re.BETA_FAMILY = 1.0 - (1.0 - SAR) ** re.GAMMA

print(f'Fan-curve run: SAR={int(SAR*100)}%  β_family={re.BETA_FAMILY:.4f}  seeds=0–{N_SEEDS-1}')
...
def run_and_track(agents_df, seed):
    ...
    active = []
    for idx in init_ids:
        agents[idx].status = agents[idx].next_status = 'I'
    ...
    return np.array(daily_new, dtype=float), 1.0 - S_final / N
...
re.BETA_FAMILY = 0.8   # restore default
